...
Reactionable's release-message handler can post comments for released fixes.
This setup uses the same detection idea but keeps the
...
- `--loaders Fabric,Forge` to bypass module-directory detection
- `--loader-order Fabric,Forge` to control link order
- `--download-platforms curseforge,modrinth` to control which download links are shown; use `none` to disable them
- `--publish-result-file Fabric=Fabric/build/publishMods/publishCurseforge.json`
