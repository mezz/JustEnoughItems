...
public class DrawableNineSliceTexture {
	...
	private static void draw(BufferBuilder bufferBuilder, Matrix4f matrix, float minU, float minV, float maxU, float maxV, int xOffset, int yOffset, int width, int height) {
		bufferBuilder.addVertex(matrix, xOffset, yOffset + height, 0).setUv(minU, maxV);
		bufferBuilder.addVertex(matrix, xOffset + width, yOffset + height, 0).setUv(maxU, maxV);
		bufferBuilder.addVertex(matrix, xOffset + width, yOffset, 0).setUv(maxU, minV);
		bufferBuilder.addVertex(matrix, xOffset, yOffset, 0).setUv(minU, minV);
	}
}
