...
public abstract class IngredientGridTooltipComponent<T> implements ClientTooltipComponent, TooltipComponent {
	...
	public void renderImage(Font font, int x, int y, PoseStack poseStack, ItemRenderer itemRenderer, int z) {
		...
		for (int i = firstIndex; i < lastIndex; i++) {
			...
			this.slotBackground.draw(poseStack, cellX, cellY);
			drawIngredient(poseStack, this.ingredients.get(i), i, cellX + 1, cellY + 1, i == hoveredIndex);
		}

		if (this.maxRowOffset > 0) {
			drawScrollbar(poseStack, gridX, gridY);
		}
	}
	...
	private void drawScrollbar(PoseStack poseStack, int gridX, int gridY) {
		...
		this.scrollbar.draw(poseStack, this.visibleRows, this.maxRowOffset, getScrollOffsetY());
	}
