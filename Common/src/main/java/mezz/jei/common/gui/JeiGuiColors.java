...
public final class JeiGuiColors {
	...
	private static final ResourceLocation COLORS_RESOURCE = new ResourceLocation(ModIds.JEI_ID, "gui/colors.json");
	...
	public static void onResourceManagerReload(ResourceManager resourceManager) {
		...
		try {
			for (Resource resource : resourceManager.getResources(COLORS_RESOURCE)) {
				loadColors(resource, loadedColors);
			}
		} catch (IOException e) {
			LOGGER.error("Failed to find JEI GUI colors resource: {}", COLORS_RESOURCE, e);
		}
		colors = Map.copyOf(loadedColors);
	}

	private static void loadColors(Resource resource, Map<GuiColor, Integer> loadedColors) {
