...
public final class Scrollbar {
	...
	public void draw(PoseStack poseStack, int visibleAmount, int hiddenAmount, float scrollOffsetY) {
		if (area.isEmpty()) {
			return;
		}
		this.background.draw(poseStack, area.x(), area.y(), area.width(), area.height());
		ImmutableRect2i markerArea = calculateMarkerArea(area, visibleAmount, hiddenAmount, scrollOffsetY);
		this.marker.draw(poseStack, markerArea.x(), markerArea.y(), markerArea.width(), markerArea.height());
	}
