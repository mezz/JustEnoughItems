...
public class TextWidget implements ITextWidget, IRecipeWidget {
	...
	public void drawWidget(PoseStack poseStack, double mouseX, double mouseY) {
		...
		for (FormattedText line : lines) {
			...
			if (shadow) {
				font.drawShadow(poseStack, charSequence, xPos, yPos, color);
			} else {
				font.draw(poseStack, charSequence, xPos, yPos, color);
			}
			yPos += lineHeight;
		}

		if (DebugConfig.isDebugGuisEnabled()) {
			Screen.fill(poseStack, 0, 0, area.width(), area.height(), 0xAAAAAA00);
		}
	}
	...
	public void getTooltip(ITooltipBuilder tooltip, double mouseX, double mouseY) {
		if (mouseX >= 0 && mouseX < area.width() && mouseY >= 0 && mouseY < area.height()) {
			...
			tooltipText.forEach(tooltip::add);
		}
	}
