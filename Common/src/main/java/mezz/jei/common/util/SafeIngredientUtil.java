...
public final class SafeIngredientUtil {
	...
	public static <T> void renderTooltip(
		...
		if (CRASHING_INGREDIENT_TOOLTIPS.contains(ingredient)) {
			...
			renderHelper.renderTooltip(screen, poseStack, errorTooltip.getElements(), x, y, font, ItemStack.EMPTY);
			return;
		}

		try {
			renderHelper.renderTooltip(screen, poseStack, tooltip.getElements(), x, y, font, itemStack);
		} catch (RuntimeException | LinkageError e) {
			CRASHING_INGREDIENT_TOOLTIPS.add(ingredient);
