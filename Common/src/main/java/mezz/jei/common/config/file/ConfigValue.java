...
public class ConfigValue<T> implements IJeiConfigValue<T>, Supplier<T> {
	...
	private final WeakList<IConfigListener<T>> configListeners = new WeakList<>();
	...
	public ConfigValue(String localizationPath, String name, T defaultValue, IJeiConfigValueSerializer<T> serializer) {
		...
		this.localizedName = new TranslatableComponent(nameKey);
		this.description = new TranslatableComponent(descriptionKey);
	...
	public void addConfigListener(IConfigListener<T> listener) {
	...
	private void notifyListeners() {
		...
		configListeners.forEach(listener -> listener.onConfigValueChanged(currentValue));
	}
}
