...
public final class JeiChatItemLinks {
	...
	public static Component parse(String rawText, Function<IngredientLink, Optional<String>> ingredientNameLookup) {
		...
		while (true) {
			...
			if (marker.start() > lastEnd) {
				result.append(new TextComponent(rawText.substring(lastEnd, marker.start())));
			}
		...
		if (lastEnd < rawText.length()) {
			result.append(new TextComponent(rawText.substring(lastEnd)));
		}
	...
	public static Optional<ITypedIngredient<?>> resolveTypedIngredient(IngredientLink link, IIngredientManager ingredientManager) {
		return getIngredientTypeForUid(link.ingredientTypeUid(), ingredientManager)
	...
	private static <T> String createLinkMarkerInternal(ITypedIngredient<T> typedIngredient, IIngredientManager ingredientManager) {
		...
		IngredientLink link = new IngredientLink(getIngredientTypeUid(ingredientType), ingredientUid);
		return createLinkMarker(link);
	}

	private static Optional<IIngredientType<?>> getIngredientTypeForUid(
		String ingredientTypeUid,
		IIngredientManager ingredientManager
	) {
		return ingredientManager.getRegisteredIngredientTypes()
			.stream()
			.filter(ingredientType -> getIngredientTypeUid(ingredientType).equals(ingredientTypeUid))
			.findFirst();
	}

	private static String getIngredientTypeUid(IIngredientType<?> ingredientType) {
	...
	private static <T> Optional<ITypedIngredient<T>> resolveTypedIngredient(
		...
		return ingredientManager.getIngredientByUid(ingredientType, ingredientUid)
			.flatMap(ingredient -> ingredientManager.createTypedIngredient(ingredientType, ingredient));
	}
	...
	private static MutableComponent createLinkComponent(IngredientLink link, Function<IngredientLink, Optional<String>> ingredientNameLookup) {
		...
		if (optionalIngredientName.isEmpty()) {
			...
			return new TextComponent("[" + ingredientUid + "]");
		}

		String ingredientName = optionalIngredientName.get();
		MutableComponent component = new TextComponent("[" + ingredientName + "]");
	...
	private static HoverEvent createTextHoverEvent(String ingredientName) {
		MutableComponent hoverText = new TextComponent(ingredientName);
	...
	private static Optional<HoverEvent> createItemHoverEvent(IngredientLink link) {
		if (!link.ingredientTypeUid().equals(getIngredientTypeUid(VanillaTypes.ITEM_STACK))) {
			return Optional.empty();
		}
