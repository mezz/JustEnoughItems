...
public interface IIngredientManager {
	 ...
	 */
	@Nullable
	default <V> IIngredientType<V> getIngredientTypeOrNull(V ingredient) {
	...
	default IClickableIngredientFactory getClickableIngredientFactory() {
		return new IClickableIngredientFactory() {
			...
			public IBuilder<?> createBuilder(@Nullable Object ingredient) {
				...
				return IIngredientManager.this.createTypedIngredient(ingredient)
					.<IBuilder<?>>map(this::createBuilder)
					.orElseGet(this::emptyBuilder);
			}
			...
			private <T> IBuilder<T> emptyBuilder() {
				return new IBuilder<>() {
					@Override
					public Optional<IClickableIngredient<T>> buildWithArea(int x, int y, int width, int height) {
						return Optional.empty();
