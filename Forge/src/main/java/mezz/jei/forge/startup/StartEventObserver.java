 ...
 *
 * JEI needs to see {@link ClientPlayerNetworkEvent.LoggedInEvent} before it is ready to start. When
...
public class StartEventObserver implements ResourceManagerReloadListener {
	...
	public void register(PermanentEventSubscriptions subscriptions) {
		subscriptions.register(EventPriority.LOWEST, ClientPlayerNetworkEvent.LoggedInEvent.class, this::onLoggingIn);
	...
	private void onLoggingIn(ClientPlayerNetworkEvent.LoggedInEvent event) {
	...
	private String getRequiredStartEventsString() {
		if (shouldWaitForRecipes()) {
			return "[%s, %s]".formatted(ClientPlayerNetworkEvent.LoggedInEvent.class.getName(), RecipesUpdatedEvent.class.getName());
		}
		return "[%s]".formatted(ClientPlayerNetworkEvent.LoggedInEvent.class.getName());
	}

	private String getMissingStartEventsString() {
		StringBuilder missingEvents = new StringBuilder("[");
		if (!observedLogin) {
			missingEvents.append(ClientPlayerNetworkEvent.LoggedInEvent.class.getName());
		}
