...
public class EventRegistration {
	...
	private static void registerClientInputHandler(RuntimeEventSubscriptions subscriptions, ClientInputHandler handler) {
		...
		subscriptions.register(ScreenEvent.MouseClickedEvent.Pre.class, event -> ForgeUserInput.fromEvent(event)
			.ifPresent(input -> {
				Screen screen = event.getScreen();
		...
		subscriptions.register(ScreenEvent.MouseReleasedEvent.Pre.class, event -> ForgeUserInput.fromEvent(event)
			.ifPresent(input -> {
				Screen screen = event.getScreen();
		...
		subscriptions.register(ScreenEvent.MouseDragEvent.Pre.class, event -> {
			Screen screen = event.getScreen();
	...
	public static void registerGuiHandler(RuntimeEventSubscriptions subscriptions, GuiEventHandler guiEventHandler) {
		subscriptions.register(ScreenEvent.DrawScreenEvent.Pre.class, event -> {
			Screen screen = event.getScreen();
