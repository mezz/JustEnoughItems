...
public final class RecipeTransferGameTests {
	...
	public static void transfersIntoEmptyItemHandlerSlot(GameTestHelper helper) {
		...
		helper.assertTrue(menu.targetSlot.getItem().is(Items.OAK_PLANKS), "Expected the ingredient in the item-handler slot");
		helper.assertTrue(menu.inventorySlot.getItem().isEmpty(), "Expected the source inventory slot to be empty");
		helper.succeed();
	}
	...
	public static void doesNotInsertIntoRejectingItemHandlerSlot(GameTestHelper helper) {
		...
		helper.assertTrue(menu.targetSlot.getItem().isEmpty(), "Expected the rejecting item-handler slot to remain empty");
		helper.assertTrue(menu.inventorySlot.getItem().is(Items.OAK_PLANKS), "Expected the source ingredient to be restored");
		helper.succeed();
	}
	...
	public static void doesNotInsertIntoCraftingResultSlot(GameTestHelper helper) {
		...
		TestMenu menu = new TestMenu(player, (testMenu, testPlayer) ->
			new ResultSlot(
				testPlayer,
				new TransientCraftingContainer(testMenu, 1, 1),
				new ResultContainer(),
				0,
				0,
				0
		...
		helper.assertTrue(menu.targetSlot.getItem().isEmpty(), "Expected the crafting result slot to remain empty");
		helper.assertTrue(menu.inventorySlot.getItem().is(Items.OAK_PLANKS), "Expected the source ingredient to be restored");
		helper.succeed();
	}
