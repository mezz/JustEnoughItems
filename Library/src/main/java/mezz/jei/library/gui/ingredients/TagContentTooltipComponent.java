...
public class TagContentTooltipComponent<T> implements ClientTooltipComponent, TooltipComponent {
	...
	public void renderImage(Font font, int x, int y, PoseStack poseStack, ItemRenderer itemRenderer, int z) {
		if (ingredients.size() <= MAX_INGREDIENTS) {
			drawIngredients(poseStack, x, y, ingredients.size());
		} else {
			final int drawCount = MAX_INGREDIENTS - 1;
			drawIngredients(poseStack, x, y, drawCount);
			if (ingredients.size() > MAX_INGREDIENTS) {
				...
				font.draw(poseStack, countString, textCenterX, textCenterY, 0xAAAAAA);
			}
		}
	}

	private void drawIngredients(PoseStack poseStack, int x, int y, int maxIngredients) {
		...
		for (int i = 0; i < ingredients.size() && i < maxIngredients; i++) {
			...
			poseStack.pushPose();
			{
				...
				renderer.render(poseStack, ingredients.get(i));
			}
			poseStack.popPose();
		}
	}
