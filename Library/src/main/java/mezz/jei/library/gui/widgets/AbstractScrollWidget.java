...
public abstract class AbstractScrollWidget implements IRecipeWidget, IJeiInputHandler {
	...
	public AbstractScrollWidget(ImmutableRect2i area) {
		this.area = area;
		this.scrollbar = new Scrollbar(calculateScrollArea(area.getWidth(), area.getHeight()));
	...
	protected abstract void drawContents(PoseStack poseStack, double mouseX, double mouseY, float scrollOffsetY);
	...
	public final void drawWidget(PoseStack poseStack, double mouseX, double mouseY) {
		this.scrollbar.draw(poseStack, getVisibleAmount(), getHiddenAmount(), scrollOffsetY);
		drawContents(poseStack, mouseX, mouseY, scrollOffsetY);
	}
