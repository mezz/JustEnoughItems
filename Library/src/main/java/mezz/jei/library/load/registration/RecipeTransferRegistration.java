...
public class RecipeTransferRegistration implements IRecipeTransferRegistration {
	...
	public <C extends AbstractContainerMenu> void addRecipeTransferHandler(Class<C> containerClass, ResourceLocation recipeCategoryUid, int recipeSlotStart, int recipeSlotCount, int inventorySlotStart, int inventorySlotCount) {
		...
		RecipeType<?> recipeType = jeiHelpers.getRecipeType(recipeCategoryUid)
			.orElseThrow(() -> new IllegalArgumentException("Unknown recipe category: " + recipeCategoryUid));
		addRecipeTransferHandler(containerClass, (RecipeType<Object>) recipeType, recipeSlotStart, recipeSlotCount, inventorySlotStart, inventorySlotCount);
	}
	...
	public <C extends AbstractContainerMenu, R> void addRecipeTransferHandler(IRecipeTransferHandler<C, R> recipeTransferHandler, ResourceLocation recipeCategoryUid) {
		ErrorUtil.checkNotNull(recipeCategoryUid, "recipeCategoryUid");
		RecipeType<R> recipeType = (RecipeType<R>) jeiHelpers.getRecipeType(recipeCategoryUid)
			.orElseThrow(() -> new IllegalArgumentException("Unknown recipe category: " + recipeCategoryUid));
		addRecipeTransferHandler(recipeTransferHandler, recipeType);
	}
	...
	public <C extends AbstractContainerMenu> void addUniversalRecipeTransferHandler(IUniversalRecipeTransferHandler<C> universalRecipeTransferHandler) {
		...
		this.recipeTransferHandlers.put(containerClass, Constants.UNIVERSAL_RECIPE_TRANSFER_TYPE, adapter);
	}
