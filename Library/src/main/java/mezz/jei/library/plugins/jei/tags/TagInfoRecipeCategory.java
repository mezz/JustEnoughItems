...
public class TagInfoRecipeCategory<R extends ITagInfoRecipe, T extends RecipeType<R>> implements IRecipeCategory<R> {
	...
	public TagInfoRecipeCategory(IGuiHelper guiHelper, T recipeType, ResourceLocation registryLocation) {
		...
		this.localizedName = new TranslatableComponent("gui.jei.category.tagInformation", registryName);
		...
		Rect2i gridArea = this.scrollGridFactory.getArea();
		this.scrollGridFactory.setPosition(
			(WIDTH - gridArea.getWidth()) / 2,
			20
		);
	}
	...
	public void setRecipe(IRecipeLayoutBuilder builder, R recipe, IFocusGroup focuses) {
		Rect2i gridArea = scrollGridFactory.getArea();
		builder.addSlot(RecipeIngredientRole.INPUT, gridArea.getX() + 1, 1)
	...
	public void draw(R recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack, double mouseX, double mouseY) {
		...
		font.draw(poseStack, tagName, nameArea.x(), nameArea.getY(), 0xFF505050);
	}
	...
	public List<Component> getTooltipStrings(R recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
		if (nameArea.contains(mouseX, mouseY)) {
			...
			return List.of(
				tagName,
				new TextComponent(tag.location().toString()).withStyle(ChatFormatting.GRAY)
			);
		}
		return List.of();
	}
	...
	public void getTooltip(ITooltipBuilder tooltip, R recipe, IRecipeSlotsView recipeSlotsView, double mouseX, double mouseY) {
		...
		Component tagName = renderHelper.getName(tag);
		tooltip.add(tagName);
		tooltip.add(new TextComponent(tag.location().toString()).withStyle(ChatFormatting.GRAY));
	}
