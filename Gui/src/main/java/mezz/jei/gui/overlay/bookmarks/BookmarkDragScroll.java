...
final class BookmarkDragScroll {
	...
	double update(ImmutableRect2i area, IngredientGridNavigationMode navigationMode, double mouseX, double mouseY) {
		...
		double elapsedSeconds = clamp((now - this.lastUpdateTime) / 1_000_000_000.0, 0, 0.05);
		...
		int rows = (int) (this.pendingPixels / IngredientGrid.INGREDIENT_HEIGHT);
		int rowPixels = rows * IngredientGrid.INGREDIENT_HEIGHT;
	...
	private static double getScrollSpeed(ImmutableRect2i area, IngredientGridNavigationMode navigationMode, double mouseX, double mouseY) {
		...
		if (mouseY < topEdge) {
			return -PIXELS_PER_SECOND * clamp((topEdge - mouseY) / edgeSize, 0, 1);
		}
		if (mouseY > bottomEdge) {
			return PIXELS_PER_SECOND * clamp((mouseY - bottomEdge) / edgeSize, 0, 1);
		}
		return 0;
	}
