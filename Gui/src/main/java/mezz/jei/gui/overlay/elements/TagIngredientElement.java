...
public class TagIngredientElement<T> extends IngredientElement<T> {
	...
	private <R> boolean showTagRecipe(IRecipesGui recipesGui, RecipeType<R> recipeType) {
	...
	static <R> Optional<R> findTagRecipe(TagKey<?> tagKey, IRecipeCategory<R> recipeCategory, Stream<R> recipes) {
		return recipes
			.filter(recipe -> tagKey.location().equals(recipeCategory.getRegistryName(recipe)))
			.findFirst();
	}
}
