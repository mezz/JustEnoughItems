...
public class ClientInputHandler {
    ...
    // [FIX] Suppress the charTyped event that follows a key press which focused the search bar
    ...
    public boolean onKeyboardKeyPressedPre(Screen screen, UserInput input) {
        if (!isContainerTextFieldFocused(screen)) {
            // [FIX] Check if any char handler had focus before handling the key press
            boolean hadFocus = charTypedHandlers.stream().anyMatch(ICharTypedHandler::hasKeyboardFocus);
            ...
            if (guiProperties != null) {
                ...
                // [FIX] If the key press caused focus to transition from off to on,
                // suppress the charTyped event so the hotkey character isn't typed into the search bar
                if (handled && !hadFocus && charTypedHandlers.stream().anyMatch(ICharTypedHandler::hasKeyboardFocus)) {
    ...
    public boolean onKeyboardCharTypedPre(Screen screen, CharacterEvent event) {
        // [FIX] If the previous key press focused the search bar, consume this charTyped event
